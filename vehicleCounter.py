...
while True:
    ...
    # background_color = frame1[count_line_position, 500]
    

    cv2.line(frame1, (1, count_line_position), (2, count_line_position), (0, 126, 155), 3)
    for (i, c) in enumerate(counterShape):
        ...
        for (x, y) in detect:
            if y < (count_line_position + offset) and y > (count_line_position - offset):
                counter += 1
                cv2.line(frame1, (1, count_line_position), (2, count_line_position), (0, 0, 0), 3)
                detect.remove((x, y))
    ...
    # cv2.imshow('FDetector', dilateada)
